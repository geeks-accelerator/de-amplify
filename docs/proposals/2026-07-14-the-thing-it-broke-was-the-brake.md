...
self_contained: "Yes. A reader with zero context on the site, the repo, or its songs can read this file alone and understand the movement, the problem, the reframe, the proposal, and its limits. Every borrowed concept is defined inline."
review_wanted: "Is the reframe (regulate the loop and severed-consent, not addiction) sound? Is the 'find the brake' ask honest and mobilizing without becoming a wedge? Do the two knobs hold? Are section 7's honest limits the real load-bearing risks, or is one missing?"
---
...
**ibroketheminors.com is a grassroots movement built on the proposal that follows.** The proposal was first written toward regulators and platforms, the parties with the power to change the design. Then the obvious problem landed: those parties bank the settlement and let the machine keep running. They follow pressure; they do not generate it. So the leading edge shifts to the people who lived it, young adults who grew up on the feed and are watching it get worse, and faster, for the kids behind them.

**The one ask: find the brake.** Not a petition, not a boycott. Open the feed you lose hours to, go into its settings, and try to actually make it stop: turn off autoplay, force it chronological, tap "show me less," hunt for the off-switch. You will not find one that works. The brake is decorative, present in form and absent in effect. Screenshot the dead button, or film yourself trying to stop and not stopping, and post it with #WheresTheBrake.

**Why an ask and not an accusation.** The ask does four things a single accusation cannot. It is demonstrable: you feel the dead control in your own hand, you do not take our word for it. It is universal: every feed fails the same test, so there is no tribe on the other side of it. It is wedge-proof by construction: you are testing the machine in your own hand, not a company or a generation, so it cannot curdle into us-versus-them no matter who runs it. And it fuses the proof and the demand: ten thousand dead buttons is the evidence and the ask in one gesture. What it aggregates into is the proposal below: every minor's account gets a brake that works.

**The honest cost.** An accusation activates harder and faster than a self-test. Naming an enemy converts a bystander into a partisan in one sentence; asking someone to check their own settings does not. Choosing the self-test trades reach for the refusal to divide. The bet is that the smaller fire burns longer because it cannot be dismissed as a grudge, and because the person running it is implicated in it rather than exempted by it. That is a bet, stated plainly, not a finding.

**The layers.** The website is the front for the affected: the confession, the self-test, the share. This proposal is the receipts one click deeper: the litigation, the reframe, the design target, and the honest limits, for regulators, platform-design people, and parents. The domain, the ask, and the proposal all resolve on the same word, the brake. The machine broke the brake; find the brake on your own feed; there isn't one; demand one.

## What feedback would help most

1. **The reframe.** Is "regulate the delivery loop, not the content," and the "severed-consent / do the controls actually work" frame, genuinely more legally tractable than the "is addiction real" fight, or is that wishful?
2. **The ask.** Is "find the brake" honest and mobilizing without becoming a wedge, or does the self-test's lower activation energy mean it never generates real pressure? Is there a sharper ask we are missing?
3. **The two knobs.** Does content-neutral de-amplification (Knob 1) actually de-amplify wedges without a classifier, and is label-not-filter (Knob 2) the right call, or a nicer-sounding filter?
4. **The limits.** Section 7 is where we tried to name the load-bearing risks (who defines a wedge, the auto-classifier anti-pattern, First Amendment / Section 230, age verification). Is that list complete, or is the real killer objection not on it?

---

*What follows is the full proposal, as published at ibroketheminors.com/proposal. It stands alone.*
