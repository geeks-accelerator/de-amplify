...
superseded: >
  SUPERSEDED AS A SOURCE on 2026-07-16, marked explicitly on 2026-07-25. This was the original
  review copy of the /lawsuits explainer, from before the lawsuit content moved to the ledger-first
  pipeline. IT IS NO LONGER A SOURCE FOR ANYTHING. `content/lawsuits.md` is now seeded from the
  per-case evidence ledgers in `docs/distillations/`, corrections land there first, and this file
  has not been kept in step; as of 2026-07-25 it differs from the live page by roughly fourteen
  passages and every one of those is the live page being MORE correct.
  DO NOT edit this file expecting the change to reach the site, and above all DO NOT regenerate
  `content/lawsuits.md` from it, which would silently revert months of ledger-first corrections
  including verdict postures, settlement dates, and quote fidelity fixes.
  The trap is that this file looks exactly like the two review copies in this folder that ARE live
  (`2026-07-15-brake-integrity-standard.md` -> `content/proposal.md`, and
  `2026-07-15-experimental-appendix-mechanism-labeling.md` -> `content/notes.md`). Those two must
  be edited first and their bodies regenerated. This one is a historical artifact, kept for the
  record of what was published on 2026-07-16 and for nothing else.
---
