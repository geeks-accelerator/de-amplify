...
15. **[ESTABLISHED]** On the under-13 question the Commission put a number on the population Meta's assessment allegedly failed to account for. It says that assessment **"contradicts large bodies of evidence from all over the European Union indicating that roughly 10-12% of children under 13 are accessing Instagram and/or Facebook."** *(IP/26/920. Emphasis is on the whole sentence, not inside it: this record does not add markup within a quoted span. This is the Commission characterising a body of evidence, not publishing a study of its own.)*
...
- **Article 27(3) compels access to a switch, not the existence of a second option.** The parent plan records that the right to select a non-profiled feed is conditional on more than one option already existing, and that the obligation to offer a non-profiled option at all is Article 38, which binds only very large platforms. **Deliberately paraphrased rather than quoted:** the plan carries the statutory wording, but the plan is a research file, not a cached primary source, and this project's rule is that no quotation enters a ledger from it. Quoting Article 27(3) here means fetching and caching the DSA text itself. None of the four releases in this record turns on Article 27 or 38, so this ledger makes no Article-level claim at all. If a future pass adds one, that distinction is the trap.
