...
- **Both judgments were read in full from the official source.** The Dutch judiciary publishes decisions as open data at `data.rechtspraak.nl/uitspraken/content?id=<ECLI>`, which returns XML including the full body. Both fetched cleanly over plain HTTPS with no blocking, which makes `rechtspraak.nl` one of the least obstructed primary sources the project has used. **Do not generalise from that into a blanket claim about which domains block fetches; an earlier version of this note did and was wrong.** Tested 2026-08-14: `nmdoj.gov` returns 403 to `curl` whatever the User-Agent, `ftc.gov` returns 403 only to a default `curl` User-Agent and 200 to a browser one, and `courtlistener.com` and `ecf.ca8.uscourts.gov` do not block `curl` at all. Test the host.
