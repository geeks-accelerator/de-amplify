...
- **Ledger snippet (~155 chars, for the /distillations/<slug> meta description ONLY; keeps the search snippet to a single consumer):** Every claim behind the MDL 3047 case file, tiered by source strength: what the court actually held in June 2026, what is only alleged, and what is Meta's own filing.
...
- **The Section 230 narrowing is unledgered (opened 2026-07-24).** The lawsuits hub states that earlier rulings in this MDL narrowed parts of the case on Section 230 grounds. That is widely reported and predates this ledger's sourcing passes, but no claim here anchors it to a specific order, so the hub now carries it as background rather than a cited holding. Next pass: locate the motion-to-dismiss order(s) on the docket and tier the narrowing properly, or drop the sentence.
- **The states' own penalty submission has not been read (opened 2026-07-24).** Every $1.4 trillion claim here traces to Meta's filing (Dkt 455) and to the Remedy Chart exhibit as described on the docket. The states' own methodology filing is not among the documents fetched, so the hub previously described the underlying filings as "sealed", which no source in this record supports; that assertion was removed. Whether those filings are sealed, unfetched, or simply unexamined is itself an open question.
