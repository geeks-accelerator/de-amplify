...
- Visible "as of" date at the top; JSON-LD `dateModified` from file mtime **(SUPERSEDED 2026-07-24: git author date via `contentDate.ts`, never mtime)**; sitemap lastmod follows automatically.
...
- [x] JSON-LD: `Article` (with dateModified from the git author date via `contentDate.ts`, **not** mtime; corrected 2026-07-24) plus `BreadcrumbList`.
...
Verified as stated (no change needed): the June 30 summary-judgment ruling (denial for Meta plus partial COPPA win for the states), the $375M New Mexico verdict and its consumer-protection framing (with the addition that an injunctive phase remains live), the ~$27M federal bellwether settlement (total better corroborated than the per-defendant split), the $1.4T figure (from Meta's own filing, which called it "extraordinary and unjustified"), the EU DSA preliminary findings dates (TikTok 2026-02-06, Meta 2026-07-10; keep the word "preliminarily"), and NetChoice v. Bonta. **(AMENDED 2026-07-24: a third EC finding belongs in this inventory, the April 29, 2026 under-13 age-assurance breach, IP/26/920. And the standing caveat this list lacks: the Brussels proceeding has **no evidence ledger**, so unlike the three U.S. cases its claims have no tiered record and no ledger-first place for a correction to land. Do not add new EC claims to the site until a ledger exists.)** Case counts: use "2,893 pending actions (JPML, July 1, 2026)", not "mid-July".
...
- **Shipped:** `/lawsuits` (rendered via the markdown pipeline from `content/lawsuits.md`), `/lawsuits.md` raw route, sitemap and llms.txt entries, Header and Footer links, homepage "the full record" link under the receipts section. Build prerenders 17/17 static; lint clean; em-dash scan zero. (Route counts as of 2026-07-16; the site has grown since, so treat the figure as a dated record rather than a current invariant.)
...
- **Post-audit corrections (2026-07-16, second pass):** a full external-validation audit of the distillations and the deployed lawsuit content (consolidated in an audit record) reached the primary court record via CourtListener/RECAP for the first time and corrected the page plus the policy paper on five points. (1) Meta's filing (Dkt 455) says "entirely unmoored" and "no analog in the history of consumer protection enforcement" verbatim; "extraordinary and unjustified" was an outlet's unquoted paraphrase, so the earlier "unmoored" correction is reversed on primary evidence; the review-round note above should be read with this reversal in mind, since the replacement wording adopted there was itself the paraphrase. (2) The summary-judgment order was filed June 29 (June 30 is the announcement date); the "public relations stunt" language is now verified in the order itself at p. 23. (3) The K.G.M. post-trial denial is stated as "early June" (Reuters reports the ruling made Tuesday June 9; June 10 was the announcement); the YouTube second-bellwether settlement was reported June 23 (June 24 was the story date) and TikTok's June 30. (4) New Mexico's posture is "Meta says it will appeal" (announced intent; no notice of appeal reported), and the "only live proceeding that could order design changes" framing is corrected to "furthest along" (the MDL AGs also seek injunctive relief, tried to the court in a post-liability remedies phase). (5) The August trial is against Meta alone, now said explicitly; the federal bellwether settlement figures are documented (not estimates) via a Reuters public-records request. The audit also confirmed the page's "up 229 in a single month" against the June 1 and July 1 JPML reports.
- **Single-source-of-truth decision RESOLVED (2026-07-16, Lee):** the lawsuit content is now **ledger-first with seeded pages**. The per-case evidence ledgers stay in `docs/distillations/` (a generic distillation home, deliberately not lawsuit-specific), and the site gained three per-case pages *seeded from* them, curated for the web reader, not a 1:1 render and not a build-time reference into `docs/`: `content/lawsuits/{mdl-3047,kgm-v-meta,new-mexico-v-meta}.md` rendered at `/lawsuits/mdl-3047`, `/lawsuits/kgm-v-meta`, `/lawsuits/new-mexico-v-meta` (each with a raw `.md` twin route, JSON-LD Article + breadcrumbs, sitemap and llms.txt entries). The `/lawsuits` hub keeps its explainer role and gained case-file links (the map section, one per case section, and the provenance note now names the ledger-first flow). The sync discipline that closes the drift risk: corrections land in the ledger first (verified against primary sources), then the case file and hub are re-seeded on a dated pass; the Companion-source note's standing risk is thereby retired. The K.G.M. slug (`kgm-v-meta`) targets the documented low-competition keyword from section 10. **TLDR surfaces wired verbatim (same day):** each ledger's TLDR block now feeds the site as exact strings, so summary surfaces cannot drift from the ledger: the one-line label → the hub's case-file cards; a new ~155-character search-snippet variant (added to each ledger for this surface) → each page's meta-description tag; the one-sentence card → each page's JSON-LD Article description; the one-paragraph variant → each page's "The case in one paragraph" opener. Facts displaced from the openers (the 2,893/+229 count, the judge) were rehomed into body sections. **(EXTENDED 2026-07-24: six variants now, not four. Two more were added to all seven ledgers, `Nav label` (~40 chars) → the `/distillations` index card and breadcrumb, and `OG title` (~50 chars) → the share card, because those two strings had been hand-authored in the `DISTILLATIONS` array in `src/lib/distillations.ts` and were therefore unvalidated third variants of summaries the ledgers already owned. That array is now routing config only. `loadDistillation` parses all of them; the hub-card label is the one variant still copied by hand into the markdown hubs.)**
