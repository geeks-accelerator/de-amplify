...
**One row of Pass 2's file list was pulled forward the same day, and only one.** `content/proposal.md` (via its review copy) had its section 1 jury-selection sentence flipped to past tense, because that section's own heading was moved from "as of mid-July 2026" to "as of August 2026" during the New Mexico revision, and a section headed August cannot say a August 12 event "begins". Nothing else in Pass 2 moved: the ledger TLDR variants, the hub card, the case-page opener, the route `DESCRIPTION` and the homepage `Stat` are all still pending and still fire on or after August 18. This is a note, not a licence to treat Pass 2 as started.
