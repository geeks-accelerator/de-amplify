...
function Group({ kind, heading, blurb }: { kind: DistillationKind; heading: string; blurb: string }) {
  // Every string on these cards is the ledger's own, parsed from its TLDR block.
  const items = DISTILLATIONS.filter((d) => d.kind === kind).map((d) => loadDistillation(d.slug));
  return (
    <section className="mt-12">
      <h2 className="font-mono text-[10px] uppercase tracking-widest2 text-bone/60">{heading}</h2>
      <p className="mt-3 text-[14px] leading-[1.7] text-bone/55">{blurb}</p>
      <ul className="mt-6 space-y-3">
        {items.map((d) => (
          <li key={d.slug}>
            <Link
              href={`/distillations/${d.slug}`}
            ...
            >
              ...
              {d.searchSnippet ? (
                <span className="mt-2 block text-[13px] leading-[1.6] text-bone/55">{d.searchSnippet}</span>
              ) : null}
            </Link>
          </li>
        ))}
